...
TRANSLATIONS = {
    ...
    "forte.table.headers": {
        "zh": ["输入集合", "Normal Order", "Prime Form", "Forte Class"],
        "en": ["Input Set", "Normal Order", "Prime Form", "Forte Class"]
    },
    ...
    "sr.info_target": {"zh": "目标: [{pc}]  Forte: {forte}  音程向量: {iv}", "en": "Target: [{pc}]  Forte: {forte}  IV: {iv}"},
    ...
    "sr.rel_item": {"zh": "[{pc}]  Forte: {forte}", "en": "[{pc}]  Forte: {forte}"},
    "sr.rel_k": {"zh": "[{pc}]  Forte: {forte}  (含于目标及其补集)", "en": "[{pc}]  Forte: {forte}  (subset of target & complement)"},
    "sr.rel_z": {"zh": "[{pc}]  Forte: {forte}  IV: {iv}", "en": "[{pc}]  Forte: {forte}  IV: {iv}"},
    ...
    "tt.btn_export_matrix": {"zh": "导出矩阵 PNG", "en": "Export Matrix PNG"},
