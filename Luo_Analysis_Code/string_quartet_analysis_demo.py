...
for i in range(0, 12, 3):
    chord_notes = aRow[i:i + 3]
    c = chord.Chord(chord_notes)
    c.addLyric(f"Prime: {c.primeFormString}")
    c.addLyric(f"Forte: {c.forteClass}")
    bStream.append(c)
# 打开预览
bStream.show()
# 保存xml+png
tri_xml = save_dir / "tritone_3chords.musicxml"
tri_png = save_dir / "tritone_3chords.png"
bStream.write("musicxml", fp=str(tri_xml))
bStream.write("musicxml.png", fp=str(tri_png))
print(f"三音和弦文件已保存：\n{tri_xml}\n{tri_png}")
...
for i in range(0, 12, 4):
    chord_notes = aRow[i:i + 4]
    c = chord.Chord(chord_notes)
    c.addLyric(f"Prime: {c.primeFormString}")
    c.addLyric(f"Forte: {c.forteClass}")
    bStream2.append(c)
bStream2.show()
tetra_xml = save_dir / "tetrachord_4chords.musicxml"
tetra_png = save_dir / "tetrachord_4chords.png"
bStream2.write("musicxml", fp=str(tetra_xml))
bStream2.write("musicxml.png", fp=str(tetra_png))
print(f"四音和弦文件已保存：\n{tetra_xml}\n{tetra_png}")

# ========== 3. 十二音拆分为2组六音和弦（步长6） ==========
print("\n==== 2组六音和弦集合 ====")
bStream3 = stream.Stream()
for i in range(0, 12, 6):
    chord_notes = aRow[i:i + 6]
    c = chord.Chord(chord_notes)
    c.addLyric(f"Prime: {c.primeFormString}")
    c.addLyric(f"Forte: {c.forteClass}")
    bStream3.append(c)
bStream3.show()
hexa_xml = save_dir / "hexachord_6chords.musicxml"
hexa_png = save_dir / "hexachord_6chords.png"
bStream3.write("musicxml", fp=str(hexa_xml))
bStream3.write("musicxml.png", fp=str(hexa_png))
print(f"六音和弦文件已保存：\n{hexa_xml}\n{hexa_png}")
